...
public class MultiplayerGameScreen extends Screen {
    ...
    public MultiplayerGameScreen(SceneManager sceneManager) {
                ...
                .addListener(
                        (obs, oldState, newState) -> {
                            if (newState == GameManager.GameState.GAME_OVER) {
                                ...
                                pause.setOnFinished(
                                        e -> sceneManager.switchScreen(ScreenType.HOME));
                                pause.play();
                            }
                        });
        ...
        Rectangle leftBorder =
                UIUtils.newRectangle(
                        GameScreen.GAME_WIDTH, GameScreen.GAME_HEIGHT, 30, GameScreen.GAME_START_Y);
        leftBorder.setFill(Color.TRANSPARENT);
        leftBorder.setStroke(Color.BLACK);
        leftBorder.setStrokeWidth(2);
        Rectangle rightBorder =
                UIUtils.newRectangle(
                        GameScreen.GAME_WIDTH,
                        GameScreen.GAME_HEIGHT,
                        Main.WINDOW_WIDTH - GameScreen.GAME_WIDTH - 30,
                        GameScreen.GAME_START_Y);
    ...
    private MapLoader.LevelData generateRandomLevel() {
        ...
        // no boss in multiplayer
        return new MapLoader.LevelData(0, "random_map", legends, bricks, new ArrayList<>(), colors);
    }
