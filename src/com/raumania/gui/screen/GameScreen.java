 ...
 *
 * <p>This screen owns a {@link GameManager} and a frame-based loop implemented via {@link
 * AnimationTimer}. When the screen starts, it initializes game objects and begins ticking the game
 * manager every frame with a computed delta time.
 */
public class GameScreen extends Screen {
     ...
     *
     * <p>The constructor also instantiates a {@link GameManager} using this screen's root pane for
     * rendering.
     *
    ...
    public GameScreen(SceneManager sceneManager) {
                ...
                .addListener(
                        (obs, oldState, newState) -> {
                            if (newState == GameManager.GameState.GAME_OVER) {
                                ...
                                pause.setOnFinished(
                                        e -> sceneManager.switchScreen(ScreenType.GAME_OVER));
        ...
        // Game border
        Rectangle border =
                UIUtils.newRectangle(GAME_WIDTH, GAME_HEIGHT, GAME_START_X, GAME_START_Y);
        ...
        // Pause screen
        // mainPause.setStyle("-fx-background-color: rgba(0, 0, 0, 0.7);"); // Add translucent black
        // background
        mainPause.setBackground(
                new Background(
                        new BackgroundFill(
                                new Color(0.0, 0.0, 0.0, 0.7), CornerRadii.EMPTY, Insets.EMPTY)));
        ...
        // Confirmation Screen
        // backChoice.setStyle("-fx-background-color: rgba(0, 0, 0, 0.7);"); // Add translucent
        // black background
     ...
     *
     * <p>Re-initializes the game state via {@link GameManager#initGame()} and creates an {@link
     * AnimationTimer} that:
     *
     * <ol>
     *   <li>Captures the current monotonic time in nanoseconds.
     *   <li>Computes the frame delta time in seconds as {@code dt = (now - lastUpdate) /
     *       1_000_000_000.0}.
     *   <li>Calls {@link GameManager#update(double)} with that {@code dt}.
     * </ol>
     *
     * On the very first tick, the loop only initializes {@link #lastUpdate} and skips update to
     * avoid a large or undefined delta time.
     */
    @Override
    public void onStart() {
     ...
     *
     * <p>This halts per-frame updates by calling {@link AnimationTimer#stop()} on the loop.
     */
    @Override
    public void onStop() {
        if (loop != null) {
    ...
    /** pause -> resume. set gameState, unfocus, start loop,set visible */
    public void resume() {
        lastUpdate = 0;
        Platform.runLater(root::requestFocus);
        loop.start();
    ...
    /** pause. set gameState, unfocus, stop loop, set visible */
    public void pause() {
        loop.stop();
        manager.setGameState(GameManager.GameState.PAUSED);
        mainPause.setVisible(true);
    ...
    private void updateCnt() {
        ...
        double arrowLeftX =
                pauseButtons.get(pauseCnt).getLayoutX()
                        - gap
        ...
        arrowLeftX =
                homeButtons.get(homeCnt).getLayoutX()
                        - gap
                        - homeChooseArrowLeft.getLayoutBounds().getWidth();
        arrowRightX =
                homeButtons.get(homeCnt).getLayoutX() + gap + homeButtons.get(homeCnt).getWidth();
    ...
    private void showTime() {
        ...
        for (Iterator<EffectCountDown> iterator = manager.getEffectCountDownList().iterator();
                iterator.hasNext(); ) {
            ...
            if (timeRemaining <= 1.5) {
                ...
                if (blink) text.setFill(Color.RED);
